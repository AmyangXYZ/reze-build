...
export function useSceneSync({
  ...
  const prev = useRef<{
    /** WHICH engine these settings were pushed to.
     *
     *  Every branch below is guarded on the setting having changed, which is
     *  right while the engine stays the same and wrong the moment it does not.
     *  A hot reload builds a NEW Engine on its own defaults — outline off, among
     *  others — while `prev` still reports every setting unchanged, so nothing
     *  is pushed and the scene silently loses whatever the defaults disagree
     *  with. Keeping the instance here turns that into a full re-sync. */
    engine: Engine
    settings: SceneSettings
    gradeSpec: GradeSpec
    backdrop: boolean
  ...
  useEffect(() => {
    ...
    const modeChanged = !p || p.engine !== engine || p.backdrop !== hasBackdrop || p.green !== exportBackground
    ...
    if (modeChanged || p!.settings.bloom !== bloom) {
    ...
    if (modeChanged || p!.settings.dof !== dof) {
    ...
    if (modeChanged || p!.settings.physics !== physics) {
      ...
      // Last: turning it off snaps every body back onto its bone, so the wind
      // and gravity written above land on the next enable rather than on a pose
