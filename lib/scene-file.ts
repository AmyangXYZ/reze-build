...
export const SCENE_FILE_KIND = "reze-build/scene-config"
 ...
 *  reze-build-<scene>-<stamp> — so a downloads folder sorts a session's artifacts
...
export function sceneZipFileName(name: string): string {
  ...
  return `reze-build-${slug(name)}-${stamp}.zip`
}
